...
public class RemoveCensorshipWordCommand extends SlashCommand {
    ...
    public void onExecute(SlashCommandInteractionEvent event) {
       ...
       boolean removed =config.removeCensorshipWords(guildId,event.getOption("word").getAsString());
        ...
        if(removed)eb.setTitle("設定成功").setDescription("已將 \""+event.getOption("word").getAsString()+"\" 移除").setColor(Color.GREEN);
        else eb.setTitle("設定失敗").setDescription("未找到 \""+event.getOption("word").getAsString()+"\" ").setColor(Color.RED);
